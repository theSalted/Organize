...
final class CaptureFolderManager: ObservableObject {
    ...
    init?() {
        ...
        relativeRoot = "Scans/" + newFolder.lastPathComponent
...
extension CaptureFolderManager {
    
    // Function to delete empty timestamped folders
    func deleteEmptyTimestampedFolders() {
        guard let scansFolder = CaptureFolderManager.rootScansFolder() else {
            logger.error("Unable to access Scans directory.")
            return
        }
        
        do {
            let directories = try FileManager.default.contentsOfDirectory(at: scansFolder,
                                                                          includingPropertiesForKeys: nil,
                                                                          options: [.skipsHiddenFiles, .skipsSubdirectoryDescendants])
            
            for directory in directories where directory.hasDirectoryPath {
                if isTimestampedDirectoryEmpty(directory) {
                    try FileManager.default.removeItem(at: directory)
                    logger.log("Deleted empty directory: \(directory.lastPathComponent)")
                }
            }
        } catch {
            logger.error("Error while trying to delete empty directories: \(error.localizedDescription)")
        }
    }
    
    // Helper function to check if a timestamped directory is empty
    private func isTimestampedDirectoryEmpty(_ directory: URL) -> Bool {
        let subdirectories = ["Images", "Snapshots", "Models"]
        
        for subdirectory in subdirectories {
            let subDirURL = directory.appendingPathComponent(subdirectory)
            if let contents = try? FileManager.default.contentsOfDirectory(at: subDirURL, includingPropertiesForKeys: nil),
               !contents.isEmpty {
                return false
            }
        }
